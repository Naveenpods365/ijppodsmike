...
export const DealCard = ({ title, discount, category, retailer }: DealCardProps) => {
  return (
    <div className="group relative bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-lg transition-all duration-500 border border-border/50 hover:border-primary/30 hover-lift">
      ...
      <div className="relative h-44 bg-gradient-to-br from-secondary via-muted to-secondary/80 overflow-hidden">
        ...
        <Badge variant="secondary" className="absolute top-3 right-3 bg-card/90 backdrop-blur-sm text-card-foreground font-medium">
          {category}
        </Badge>
      ...
      <div className="p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">{retailer}</p>
        </div>
        <h4 className="font-bold text-card-foreground line-clamp-2 leading-snug group-hover:text-primary transition-colors duration-300">{title}</h4>
        <Button 
          variant="outline" 
          size="sm" 
