...
import { useCouponsMetricsWebSocket, CouponsMetrics } from './useCouponsMetricsWebSocket';
...
describe('useCouponsMetricsWebSocket', () => {
  it('should initialize with default metrics and disconnected state', () => {
    const { result } = renderHook(() => useCouponsMetricsWebSocket());
  ...
  it('should connect WebSocket when token is present in localStorage', () => {
    ...
    const mockWs = createMockWebSocket('wss://dealscraper.rohans.uno/api/ws/coupons/metrics?token=test-token');
  ...
  it('should update metrics when WebSocket receives a message', () => {
    const mockToken = 'test-token';
    
    const mockMetrics: CouponsMetrics = {
      total_coupons: 100,
      active_coupons: 80,
      total_uses: 500,
      avg_discount: 15.5,
